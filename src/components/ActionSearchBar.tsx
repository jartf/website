...
export function ActionSearchBar() {
  ...
  const allActions = useMemo(() => {
    ...
    const navItems: Action[] = [
      { id: 'home', label: t('nav.home', 'Home'), icon: <Home className="h-4 w-4 text-primary" />, shortcut: 'h', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/') },
      { id: 'about', label: t('nav.about', 'About'), icon: <User className="h-4 w-4 text-primary" />, shortcut: 'a', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/about/') },
      { id: 'projects', label: t('nav.projects', 'Projects'), icon: <Code className="h-4 w-4 text-primary" />, shortcut: 'p', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/projects/') },
      { id: 'blog', label: t('nav.blog', 'Blog'), icon: <BookOpen className="h-4 w-4 text-primary" />, shortcut: 'b', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/blog/') },
      { id: 'now', label: t('nav.now', 'Now'), icon: <Clock className="h-4 w-4 text-primary" />, shortcut: 'n', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/now/') },
      { id: 'uses', label: t('nav.uses', 'Uses'), icon: <Wrench className="h-4 w-4 text-primary" />, shortcut: 'u', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/uses/') },
      { id: 'contact', label: t('nav.contact', 'Contact'), icon: <Mail className="h-4 w-4 text-primary" />, shortcut: 'c', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/contact/') },
      { id: 'guestbook', label: t('nav.guestbook', 'Guestbook'), icon: <MessagesSquare className="h-4 w-4 text-primary" />, shortcut: 'g', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/guestbook/') },
      { id: 'colophon', label: t('nav.colophon', 'Colophon'), icon: <FileText className="h-4 w-4 text-primary" />, shortcut: 'l', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/colophon/') },
      { id: 'webring', label: t('nav.webring', 'Webrings'), icon: <FlipHorizontal className="h-4 w-4 text-primary" />, shortcut: 'w', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/webrings/') },
      { id: 'scrapbook', label: t('nav.scrapbook', 'Scrapbook'), icon: <Calendar className="h-4 w-4 text-primary" />, shortcut: 'd', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/scrapbook/') },
      { id: 'slashes', label: 'Slashes', icon: <Slash className="h-4 w-4 text-primary" />, shortcut: '/', category: t('keyboardShortcuts.navigation', 'Navigation'), action: nav('/slashes/') },
    ]
    ...
    const generalActions: Action[] = [
      { id: 'refresh-cat', label: t('keyboardShortcuts.refreshCat', 'Refresh mood cat'), icon: <RefreshCw className="h-4 w-4 text-purple-500" />, shortcut: 'r', category: t('actionSearch.fun', 'Fun'), action: () => { document.querySelector('button[aria-label="Refresh mood cat"]')?.dispatchEvent(new MouseEvent('click', { bubbles: true })); close() } },
      ...
      { id: 'keyboard-shortcuts', label: t('keyboardShortcuts.openShortcuts', 'Keyboard shortcuts'), icon: <KeyRound className="h-4 w-4 text-gray-500" />, shortcut: ',', category: t('actionSearch.help', 'Help'), action: () => { dispatchKey(','); close() } },
    ]

    // Project page actions
    const projectActions: Action[] = !isPage('/projects') ? [] : [
      { id: 'close-card', label: t('actionSearch.projects.closeCard', 'Close card'), icon: <X className="h-4 w-4 text-red-500" />, shortcut: 'Esc', category: t('actionSearch.projects.category', 'Projects'), action: () => { document.querySelector('.rotate-y-180 [id^="project-card-back-"]')?.dispatchEvent(new MouseEvent('click', { bubbles: true })); close() }, showOn: ['/projects'] },
      { id: 'nav-left', label: t('actionSearch.projects.left', 'Navigate left'), icon: <ArrowLeft className="h-4 w-4 text-blue-500" />, shortcut: '←', category: t('actionSearch.projects.category', 'Projects'), action: () => { dispatchKey('ArrowLeft'); close() }, showOn: ['/projects'] },
      { id: 'nav-right', label: t('actionSearch.projects.right', 'Navigate right'), icon: <ArrowRight className="h-4 w-4 text-blue-500" />, shortcut: '→', category: t('actionSearch.projects.category', 'Projects'), action: () => { dispatchKey('ArrowRight'); close() }, showOn: ['/projects'] },
      { id: 'nav-up', label: t('actionSearch.projects.up', 'Navigate up'), icon: <ArrowUp className="h-4 w-4 text-blue-500" />, shortcut: '↑', category: t('actionSearch.projects.category', 'Projects'), action: () => { dispatchKey('ArrowUp'); close() }, showOn: ['/projects'] },
      { id: 'nav-down', label: t('actionSearch.projects.down', 'Navigate down'), icon: <ArrowDown className="h-4 w-4 text-blue-500" />, shortcut: '↓', category: t('actionSearch.projects.category', 'Projects'), action: () => { dispatchKey('ArrowDown'); close() }, showOn: ['/projects'] },
    ]
    ...
    const blogActions: Action[] = !isPage('/blog') ? [] : pathname !== '/blog' && pathname !== '/blog/' ? [
      ...
      { id: 'blog-search', label: t('actionSearch.blog.search', 'Search'), icon: <Search className="h-4 w-4 text-blue-500" />, shortcut: 's', category: t('actionSearch.blog.category', 'Blog'), action: () => { (document.querySelector('input[placeholder*="Search"]') as HTMLInputElement)?.focus(); close() }, showOn: ['/blog'] },
      { id: 'blog-filter', label: t('actionSearch.blog.filterTag', 'Filter'), icon: <Tag className="h-4 w-4 text-green-500" />, category: t('actionSearch.blog.category', 'Blog'), action: () => { document.querySelector('button:has(.lucide-filter)')?.dispatchEvent(new MouseEvent('click', { bubbles: true })); close() }, showOn: ['/blog'] },
    ]
    ...
    const CATEGORY_ICONS: Record<string, any> = { reading: BookOpen, coding: Code, drinking: Coffee, listening: Headphones, thinking: Brain, studying: GraduationCap, planning: Lightbulb }
    const nowActions: Action[] = !isPage('/now') ? [] : nowCategories.map((cat, idx) => {
      ...
      return {
        ...
        action: () => { scrollTo(`category-${cat}`); close() },
        showOn: ['/now'],
      }
    })
    ...
    const usesActions: Action[] = !isPage('/uses') ? [] : usesCategories.map((cat, idx) => {
      ...
      return {
        ...
        action: () => { scrollTo(`[id^="category-"]:nth-of-type(${idx + 1})`); close() },
        showOn: ['/uses'],
      }
    })
    ...
    const colophonActions: Action[] = !isPage('/colophon') ? [] : colophonSections.map((sec, idx) => {
      ...
      return {
        ...
        action: () => { scrollTo(`[id^="section-"]:nth-of-type(${idx + 1})`); close() },
        showOn: ['/colophon'],
      }
    })
  ...
  const groupedActions = useMemo(() => filteredActions.reduce((acc, a) => {
    ...
    return acc
  }, {} as Record<string, Action[]>), [filteredActions])
